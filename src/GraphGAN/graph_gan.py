...
import src.evaluation.eval_link_prediction as elp
...
class GraphGan(object):
    def __init__(self):
        ...
        self.node_embed_init_d = utils.read_emd(filename=config.pretrain_emd_filename_d, n_node=config.n_node,
                                                n_embed=config.n_embed)
        self.node_embed_init_g = utils.read_emd(filename=config.pretrain_emd_filename_g, n_node=config.n_node,
                                                n_embed=config.n_embed)
        ...
        for epoch in tqdm.tqdm(range(config.max_epochs)):
            ...
            for d_epoch in tqdm.tqdm(range(config.max_epochs_dis)):
                ...
                while True:
                    ...
                    _, loss = self.sess.run([self.discriminator.d_updates, self.discriminator.pre_loss],
                                            {self.discriminator.q_node: np.array(input_q_node), \
                                             self.discriminator.rel_node: np.array(input_rel_node),
                                             self.discriminator.label: np.array(input_label)})
            ...
            for g_epoch in tqdm.tqdm(range(config.max_epochs_gen)):
                ...
                trace = []  # the trace when sampling the nodes, from the root to leaf  bach to leaf's father. e.g.:
                # 0 - 1 - 2 -1
                all_score = self.sess.run(
                    self.generator.all_score)  # compute the score for computing the probability when sampling nodes
                for root_node in tqdm.tqdm(self.root_nodes, mininterval=3):  # random update trees
                    if np.random.rand() < config.update_ratio:
                        ...
                        if cnt % config.gen_update_iter == 0 and cnt > 0:
                            ...
                            pairs = list(map(self.generate_window_pairs,
                                             trace))  # [[], []] each list contains the pairs along the same path
                            q_node_gen = []
                            rel_node_gen = []
                            ...
                            reward_gen, node_embed = self.sess.run(
                                [self.discriminator.score, self.discriminator.node_embed],
                                {self.discriminator.q_node: np.array(q_node_gen),
                                 self.discriminator.rel_node: np.array(rel_node_gen)})

                            feed_dict = {self.generator.q_node: np.array(q_node_gen),
                                         self.generator.rel_node: np.array(rel_node_gen),
                                         self.generator.reward: reward_gen}
                            _, loss, prob = self.sess.run(
                                [self.generator.gan_updates, self.generator.gan_loss, self.generator.i_prob],
                                feed_dict=feed_dict)
                            all_score = self.sess.run(self.generator.all_score)
                            root_nodes = []
                            rel_nodes = []
                            ...
                            trace = []
                            cnt = 0
                        sample = self.sample_for_gan(root_node, self.trees[root_node], config.n_sample_gen, all_score,
                                                     sample_for_dis=False)
    ...
    def generate_window_pairs(self, sample_path):
        """
        given a sample path list from root to a sampled node, generate all the pairs corresponding to the windows
        size e.g.: [1, 0, 2, 4, 2], window_size = 2 -> [1, 0], [1, 2], [0, 1], [0, 2], [0, 4], [2, 1], [2, 0], [2,
        4], [4, 0], [4, 2] :param sample_path: :return:
        """
    ...
    def eval_test(self):
        ...
        if config.app == "link_prediction":
            for i in range(2):
                LPE = elp.LinkPredictEval(config.emb_filenames[i], config.test_filename, config.test_neg_filename,
